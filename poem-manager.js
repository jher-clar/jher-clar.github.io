...
async function loadPoems() {
    ...
    // Merge and deduplicate poems, prioritizing local storage (latest user poem with same score)
    const mergedPoems = new Map();

    // Add initial poems
    initialPoems.forEach(poem => mergedPoems.set(`${poem.author}-${poem.poem}`, poem));

    // Add/replace with localStorage poems
    localStoragePoems.forEach(poem => {
        const key = `${poem.author}-${poem.poem}`
        if (mergedPoems.has(key)) {
            const existingPoem = mergedPoems.get(key);
            if(existingPoem.score === poem.score){
                mergedPoems.set(key, poem);
            }
        } else {
            mergedPoems.set(key, poem)
        }
    })
    const allPoems = Array.from(mergedPoems.values())

    // Sort poems by score
    allPoems.sort((a, b) => b.score - a.score);

    // Clear current poem list and add title.
    topPoemsList.innerHTML = '<h2>🏆 Top Poems</h2>';
    const poemsToDisplay = allPoems.length > 0 ? allPoems.slice(0, 5) : initialPoems.slice(0,3)
