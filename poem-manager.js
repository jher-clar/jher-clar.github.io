...
async function loadPoems() {
    ...
    // Populate top 3 poems in topPoemsList
    const topPoems = allPoems.slice(0, 3); // Get the top 3 poems
    topPoems.forEach((poem) => {
        const card = createPoemCard(poem, true);
        topPoemsList.appendChild(card);
    });

    // Populate the remaining poems (4th and 5th) or less then 5 in poemList
    const remainingPoems = allPoems.length > 5 ? allPoems.slice(3,5) : (allPoems.length < 3 ? [] : allPoems.slice(3));
